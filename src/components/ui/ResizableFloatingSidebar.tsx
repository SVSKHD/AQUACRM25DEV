...
export default function ResizableFloatingSidebar({
  ...
  useEffect(() => {
    ...
    if (Number.isFinite(saved) && saved > 0) {
      setWidth(clamp(saved, minWidth, viewportMaxWidth()));
    }
  ...
  useEffect(() => {
    ...
    const onResize = () => {
      setWidth((current) => clamp(current, minWidth, viewportMaxWidth()));
    };
  ...
  const resize = (event: ReactPointerEvent<HTMLButtonElement>) => {
    ...
    setWidth(
      clamp(startWidthRef.current + delta, minWidth, viewportMaxWidth()),
    );
  };
      ...
      >
        <LiquidPanel className="relative flex h-full min-h-0 flex-col overflow-hidden border-white/20 shadow-2xl">
          <button
            ...
            className="group absolute inset-y-0 left-0 z-30 hidden w-4 -translate-x-1/2 cursor-col-resize touch-none items-center justify-center md:flex"
          >
            <span className="flex h-20 w-3 items-center justify-center rounded-full border border-white/20 bg-slate-800/75 text-white shadow-xl backdrop-blur-xl transition group-hover:bg-sky-600">
