...
type CustomerProfile = {
  ...
  reviewsCount: number;
  invoiceLoginLinked: boolean;
  invoiceProfileEnrichedAt: string;
  addresses: any[];
  orders: any[];
  invoices: any[];
  reviews: any[];
...
const money = (...values: any[]) =>
  Number(values.find((value) => Number(value) > 0) || 0);
...
const getQuantity = (item: any) =>
  Number(item.productQuantity || item.quantity || item.qty || 1);
const getPrice = (item: any) =>
  Number(item.productPrice || item.price || item.salePrice || 0);

const normalizeProfile = (
  raw: any,
  fallbackSource: SourceTab,
): CustomerProfile => {
  const customer = raw.customer || raw;
  const source =
    (raw.source || raw.customerSource || fallbackSource) === "offline"
      ? "offline"
      : "online";
  ...
  return {
    ...
    ismobileLoginConfirmation: Boolean(
      customer.ismobileLoginConfirmation ?? raw.ismobileLoginConfirmation,
    ),
    ...
    totalSpent: money(
      ...
      raw.total_revenue,
      raw.total_amount,
      raw.stats?.totalSpent,
    ),
    ordersCount: Number(
      raw.ordersCount ?? raw.stats?.ordersCount ?? orders.length ?? 0,
    ),
    invoicesCount: Number(
      raw.invoicesCount ?? raw.stats?.invoicesCount ?? invoices.length ?? 0,
    ),
    reviewsCount: Number(
      raw.reviewsCount ?? raw.stats?.reviewsCount ?? reviews.length ?? 0,
    ),
    invoiceLoginLinked: Boolean(
      customer.firebaseUid ||
      customer.isGoogleLogin ||
      customer.invoiceProfileEnrichedAt ||
      raw.invoiceLoginLinked,
    ),
    invoiceProfileEnrichedAt: text(
      customer.invoiceProfileEnrichedAt,
      raw.invoiceProfileEnrichedAt,
    ),
    addresses,
    orders,
    invoices,
...
const Field = ({
  ...
  <label className="block">
    <span className="mb-1.5 block text-xs font-bold uppercase tracking-wide text-slate-500 dark:text-white/45">
      {label}
    </span>
    {children}
  </label>
...
export default function CustomersTab({ activeSource }: CustomersTabProps) {
  ...
  const loadCustomers = async (source: SourceTab = activeSource) => {
    setLoading(true);
    const response: any = await customerProfilesService.list({
    ...
    setCustomers(
      unwrapArray(response.data).map((item) => normalizeProfile(item, source)),
    );
    setLoading(false);
  };
  ...
  const openDetails = async (
    ...
    setCustomers((prev) =>
      prev.map((item) => (item.id === customer.id ? detail : item)),
    );
    setDetailLoading(false);
  };
  ...
  const buildUserPayload = () => {
    if ((editingCustomer?.source || activeSource) === "offline") {
      return {
        ...
        address:
          userForm.address ||
          [userForm.street, userForm.city, userForm.state, userForm.postalCode]
            .filter(Boolean)
            .join(", "),
      };
    }
  ...
  const saveUser = async (event?: FormEvent) => {
    ...
    const response: any = editingCustomer
      ? editingCustomer.source === "online"
        ? await customerProfilesService.updateOnline(
            editingCustomer.id,
            payload,
          )
        : await customerProfilesService.updateOffline(
            editingCustomer.key,
            payload,
          )
      : activeSource === "online"
  ...
  const deleteProfile = async (customer: CustomerProfile) => {
    const message =
      customer.source === "offline"
        ? "Delete this offline customer and matching invoices?"
  ...
  const saveAddress = async (event?: FormEvent) => {
    ...
    const response: any = editingAddressId
      ? await customerProfilesService.updateOnlineAddress(
          selectedCustomer.id,
          editingAddressId,
          addressForm,
        )
      : await customerProfilesService.createOnlineAddress(
          selectedCustomer.id,
          addressForm,
        );
    if (response.error) {
  ...
  const deleteAddress = async (address: any) => {
    ...
    const response: any = await customerProfilesService.deleteOnlineAddress(
      selectedCustomer.id,
      text(address.id, address._id),
    );
    if (response.error) {
  ...
  const saveReview = async (event?: FormEvent) => {
    ...
    const response: any = editingReviewId
      ? selectedCustomer.source === "online"
        ? await customerProfilesService.updateOnlineReview(
            selectedCustomer.id,
            editingReviewId,
            payload,
          )
        : await customerProfilesService.updateOfflineReview(
            selectedCustomer.key,
            editingReviewId,
            payload,
          )
      : selectedCustomer.source === "online"
        ? await customerProfilesService.createOnlineReview(
            selectedCustomer.id,
            payload,
          )
        : await customerProfilesService.createOfflineReview(
            selectedCustomer.key,
            payload,
          );
    if (response.error) {
  ...
  const deleteReview = async (review: any) => {
    ...
    const response: any =
      selectedCustomer.source === "online"
        ? await customerProfilesService.deleteOnlineReview(
            selectedCustomer.id,
            reviewId,
          )
        : await customerProfilesService.deleteOfflineReview(
            selectedCustomer.key,
            reviewId,
          );
    if (response.error) {
  ...
  return (
    <TabInnerContent
      title="Customers"
      description="Online ecommerce users and offline invoice customers"
    >
      <div className="mb-6 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        >
          ...
          {activeSource === "online" ? "Online User" : "Offline Customer"}
        </button>
      </div>
            ...
            <table className="w-full min-w-[980px] border-collapse text-left">
              <thead>
                <tr className="border-b border-slate-200 text-xs uppercase tracking-wide text-slate-500 dark:border-white/10 dark:text-white/40">
                  ...
                  <th className="px-4 py-3">Invoice Login</th>
              ...
              <tbody>
                {filteredCustomers.map((customer) => (
                  ...
                  >
                    ...
                    <td
                      className="max-w-[260px] truncate px-4 py-4 text-sm text-slate-700 dark:text-white/70"
                    ...
                    <td className="px-4 py-4">
                      <InvoiceLoginBadge
                        linked={customer.invoiceLoginLinked}
                        enrichedAt={customer.invoiceProfileEnrichedAt}
                      />
                    </td>
                    <td className="px-4 py-4">
                      <div className="flex flex-wrap gap-2 text-xs font-bold">
                        <button
                          ...
                          className="rounded-full bg-emerald-500/10 px-3 py-1 text-emerald-600 dark:text-emerald-300"
                        >
                          {customer.ordersCount} Orders
                        </button>
                        <button
                          onClick={() => openDetails(customer, "invoices")}
                          className="rounded-full bg-blue-500/10 px-3 py-1 text-blue-600 dark:text-blue-300"
                        >
                          {customer.invoicesCount} Invoices
                        </button>
                        <button
                          onClick={() => openDetails(customer, "reviews")}
                          className="rounded-full bg-amber-500/10 px-3 py-1 text-amber-600 dark:text-amber-300"
                        >
                    ...
                    <td className="px-4 py-4">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          ...
                          title="View details"
                          className="rounded-xl bg-sky-50 p-2 text-sky-600 hover:bg-sky-100 dark:bg-sky-500/10"
                        >
                        ...
                        {customer.phone && (
                          <button
                            ...
                            title="WhatsApp"
                            className="rounded-xl bg-emerald-50 p-2 text-emerald-600 hover:bg-emerald-100 dark:bg-emerald-500/10"
                          >
                        ...
                        <button
                          ...
                          title="Edit"
                          className="rounded-xl bg-slate-100 p-2 text-slate-600 hover:bg-slate-200 dark:bg-white/5 dark:text-white/70"
                        >
                          <Edit2 className="h-4 w-4" />
                        </button>
                        <button
                          onClick={() => deleteProfile(customer)}
                          title="Delete"
                          className="rounded-xl bg-rose-50 p-2 text-rose-600 hover:bg-rose-100 dark:bg-rose-500/10"
                        >
      ...
      <AnimatePresence>
        {showUserForm && (
          <UserFormModal
            source={editingCustomer?.source || activeSource}
        ...
        >
          {online ? (
            <>
              ...
              <Field label="Alternative Email">
                <input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, alternativeEmail: e.target.value })
                  }
                  className={inputClass}
                />
              </Field>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <CheckField
                  ...
                  checked={form.isEmailVerfied}
                  onChange={(checked) =>
                    setForm({ ...form, isEmailVerfied: checked })
                  }
                />
                <CheckField
                  label="Mobile verified"
                  checked={form.ismobileLoginConfirmation}
                  onChange={(checked) =>
                    setForm({ ...form, ismobileLoginConfirmation: checked })
                  }
                />
              </div>
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  ...
                  <Field label="GST Email">
                    <input
                      ...
                      value={form.gstEmail}
                      onChange={(e) =>
                        setForm({ ...form, gstEmail: e.target.value })
                      }
                      className={inputClass}
                    />
                  </Field>
                  <Field label="GST Phone">
                    <input
                      value={form.gstPhone}
                      onChange={(e) =>
                        setForm({ ...form, gstPhone: e.target.value })
                      }
                      className={inputClass}
                    />
                  </Field>
                  <Field label="GST Address">
                    <input
                      value={form.gstAddres}
                      onChange={(e) =>
                        setForm({ ...form, gstAddres: e.target.value })
                      }
                      className={inputClass}
                    />
                  </Field>
            ...
            <>
              ...
              <div className="sm:col-span-2">
                <Field label="Invoice Customer Address">
                  <textarea
                    value={form.address}
                    onChange={(e) =>
...
function CustomerDetailModal(props: any) {
  ...
  return (
    ...
    >
      <motion.div
        className="flex h-[94vh] w-full max-w-6xl flex-col overflow-hidden rounded-t-3xl border border-slate-200 bg-white shadow-2xl dark:border-white/10 dark:bg-slate-950 sm:h-[88vh] sm:rounded-3xl"
      ...
      >
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                <p className="text-xs font-bold uppercase tracking-wide text-sky-500">
                  {customer.source} Customer Profile
                </p>
            ...
            <button
              onClick={onClose}
              className="rounded-2xl border border-slate-200 bg-white p-2 text-slate-500 dark:border-white/10 dark:bg-white/5 dark:text-white/60"
            >
              <X className="h-5 w-5" />
            </button>
          </div>
        ...
        <div className="custom-scrollbar flex-1 overflow-y-auto p-4 sm:p-5">
          {loading && (
            <div className="mb-4 rounded-2xl border border-sky-200 bg-sky-50 p-3 text-sm font-bold text-sky-700 dark:border-sky-400/20 dark:bg-sky-400/10 dark:text-sky-200">
          ...
          {activeTab === "orders" && (
            <RecordCards
              ...
              empty="No ecommerce orders found."
              type="order"
              onCopy={props.onCopy}
              onWhatsApp={() =>
                customer.phone && props.onWhatsApp(customer.phone)
              }
            />
          )}
          {activeTab === "invoices" && (
            <RecordCards
              records={customer.invoices}
...
function ProfileTab({
  ...
  const tiles = [
    ...
    [
      Link2,
      "Invoice Login",
      customer.invoiceLoginLinked ? "Linked" : "Not linked",
    ],
    [
      Package,
      "Profile Enriched",
      customer.invoiceProfileEnrichedAt
        ? safeDate(customer.invoiceProfileEnrichedAt)
        : "Pending",
    ],
    ...
    <div className="space-y-5">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
        {tiles.map(([Icon, label, value]: any) => (
          ...
          >
            <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-slate-500 dark:text-white/40">
              <Icon className="h-4 w-4" />
              {label}
            </div>
            <p className="mt-2 break-words text-sm font-bold text-neutral-950 dark:text-white">
              {value || "-"}
            </p>
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
        ...
        <InfoCard
          ...
          value={
            [
              customer.gstDetails?.gstNo,
              customer.gstDetails?.gstEmail,
              customer.gstDetails?.gstPhone,
              customer.gstDetails?.gstAddres,
            ]
              .filter(Boolean)
              .join(" · ") || "No GST details"
          }
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          className="rounded-2xl bg-sky-500 px-4 py-2 text-sm font-bold text-white"
        >
          <Edit2 className="mr-1 inline h-4 w-4" /> Edit Profile
        </button>
        {customer.phone && (
          <button
        ...
        {customer.email && (
          <button
            onClick={() => onCopy(customer.email)}
            className="rounded-2xl border border-slate-200 px-4 py-2 text-sm font-bold text-slate-600 dark:border-white/10 dark:text-white/70"
          >
            <Copy className="mr-1 inline h-4 w-4" /> Copy Email
          </button>
        )}
        <button
          onClick={onDeleteProfile}
          className="rounded-2xl bg-rose-500 px-4 py-2 text-sm font-bold text-white"
        >
...
function InvoiceLoginBadge({
  ...
  return (
    <span
      title={
        enrichedAt ? `Profile enriched ${safeDate(enrichedAt)}` : undefined
      }
      className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-bold ${
        linked
          ? "bg-emerald-500/10 text-emerald-700 dark:text-emerald-300"
          : "bg-slate-500/10 text-slate-600 dark:text-white/50"
      }`}
    >
      <Link2 className="h-3.5 w-3.5" />
      {linked ? "Linked & enriched" : "Not linked"}
    </span>
  );
}
...
function InfoCard({ title, value }: { title: string; value: string }) {
  return (
    <div className="rounded-2xl border border-slate-200 bg-white/80 p-4 dark:border-white/10 dark:bg-white/[0.03]">
      <p className="text-xs font-bold uppercase tracking-wide text-slate-500 dark:text-white/40">
        {title}
      </p>
  ...
  return (
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_380px]">
      <div className="space-y-3">
        {customer.source === "offline" && (
          <div className="rounded-2xl border border-amber-200 bg-amber-50 p-3 text-sm font-semibold text-amber-700 dark:border-amber-400/20 dark:bg-amber-400/10 dark:text-amber-200">
            Offline customer address is controlled by the invoice customer
            profile. Use Edit Profile.
          </div>
        )}
        {customer.addresses.length === 0 ? (
          ...
          customer.addresses.map((address: any, index: number) => (
            <div
              key={text(address.id, address._id, index)}
              className="rounded-2xl border border-slate-200 bg-white/80 p-4 dark:border-white/10 dark:bg-white/[0.03]"
            >
                ...
                {customer.source === "online" && (
                  <div className="flex gap-2">
                    <button
                      ...
                      className="rounded-xl bg-slate-100 p-2 text-slate-600 dark:bg-white/5 dark:text-white/70"
                    >
                      <Edit2 className="h-4 w-4" />
                    </button>
                    <button
                      onClick={() => onDeleteAddress(address)}
                      className="rounded-xl bg-rose-50 p-2 text-rose-600 dark:bg-rose-500/10"
                    >
                      <Trash2 className="h-4 w-4" />
                    </button>
                  </div>
      ...
      {customer.source === "online" && (
        ...
        >
          ...
          <div className="space-y-3">
            ...
            <CheckField
              label="Make selected address"
              checked={addressForm.makeSelected}
              onChange={(checked) =>
                setAddressForm({ ...addressForm, makeSelected: checked })
              }
            />
            <div className="flex gap-2">
              <button
                type="submit"
                className="flex-1 rounded-2xl bg-sky-500 px-4 py-2 text-sm font-bold text-white"
              >
                Save Address
              </button>
              {editingAddressId && (
    ...
    <div className="space-y-3">
      {records.length === 0 ? (
        ...
        records.map((record: any, index: number) => {
          const id =
            type === "order" ? getOrderId(record) : getInvoiceId(record);
          const total = money(
            record.total_amount,
            record.totalAmount,
            record.grandTotal,
          ...
          return (
            <div
              key={text(record.id, record._id, id, index)}
              className="rounded-2xl border border-slate-200 bg-white/80 p-4 dark:border-white/10 dark:bg-white/[0.03]"
            >
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  <p className="text-xs font-bold uppercase tracking-wide text-slate-500 dark:text-white/40">
                    {type === "order" ? "Order ID" : "Invoice ID"}
                  </p>
                  <p className="mt-1 break-all text-sm font-black text-neutral-950 dark:text-white">
                    {id || "-"}
                  </p>
                  <p className="mt-1 text-xs text-slate-500 dark:text-white/50">
                    {safeDate(
                      text(record.date, record.createdAt, record.created_at),
                    )}{" "}
                    ·{" "}
                    {text(
                      record.status,
              ...
              {products.length > 0 && (
                <div className="mt-4 rounded-2xl border border-slate-100 bg-slate-50 p-3 dark:border-white/5 dark:bg-black/10">
                  <p className="mb-2 text-xs font-bold uppercase tracking-wide text-slate-500 dark:text-white/40">
                    Products
                  </p>
                    ...
                    {products.map((product: any, productIndex: number) => (
                      ...
                      >
                        <span className="font-semibold text-neutral-950 dark:text-white">
                          {getProductName(product)} × {getQuantity(product)}
                        </span>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <button
                  onClick={() => onCopy(id)}
                  className="rounded-xl border border-slate-200 px-3 py-2 text-xs font-bold text-slate-600 dark:border-white/10 dark:text-white/60"
                >
                  <Copy className="mr-1 inline h-3.5 w-3.5" /> Copy ID
                </button>
                <button
                  onClick={onWhatsApp}
  ...
  return (
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_380px]">
      <div className="space-y-3">
        {customer.reviews.length === 0 ? (
          ...
          customer.reviews.map((review: any, index: number) => (
            <div
              key={text(getReviewId(review), index)}
              className="rounded-2xl border border-slate-200 bg-white/80 p-4 dark:border-white/10 dark:bg-white/[0.03]"
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-xs text-slate-500 dark:text-white/50">
                    {safeDate(
                      text(review.created_at, review.createdAt, review.date),
                    )}
                  </p>
                </div>
              ...
              <div className="mt-3 flex gap-2">
                <button
                  ...
                  className="rounded-xl bg-slate-100 px-3 py-2 text-xs font-bold text-slate-600 dark:bg-white/5 dark:text-white/70"
                >
                  <Edit2 className="mr-1 inline h-3.5 w-3.5" /> Edit
                </button>
                <button
                  onClick={() => onDeleteReview(review)}
                  className="rounded-xl bg-rose-50 px-3 py-2 text-xs font-bold text-rose-600 dark:bg-rose-500/10"
                >
      ...
      >
        ...
        <div className="space-y-3">
          <Field
            label={customer.source === "offline" ? "Invoice ID" : "Product ID"}
          >
            <input
              required={customer.source === "online"}
              value={
                customer.source === "offline"
                  ? reviewForm.invoiceId
                  : reviewForm.productId
              }
              onChange={(e) =>
                customer.source === "offline"
                  ? setReviewForm({ ...reviewForm, invoiceId: e.target.value })
